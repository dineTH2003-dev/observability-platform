...
This document presents the comprehensive, production-grade unit testing documentation for the **AI-Powered Observability Platform**, consolidated on the dedicated `test` branch. The testing suite spans all three core architectural tiers of the application:
1. **Backend REST API & Microservice Controller Layer (Node.js / Express)**
2. **Machine Learning Anomaly Detection Engine (Python / FastAPI)**
3. **Frontend Dashboard UI Utilities (React / TypeScript)**
...
|---|---|---|---|---|---|---|---|
| **Backend API** | Jest | Node.js (v18+) | **22** | **155** | **PASS** | Controllers ≥ 95% | **99.31% Lines / 100% Funcs** |
...
| **PLATFORM TOTAL** | | | **29** | **332** | **PASS** | **100% Quality Gate** | **100% Test Pass Rate** |
...
│   (Jest - Node.js 18)    │   │   (pytest - Python 3)    │   │ (Vitest - happy-dom)     │
├──────────────────────────┤   ├──────────────────────────┤   ├──────────────────────────┤
│ • 22 Test Suites         │   │ • 4 Test Suites          │   │ • 3 Test Suites          │
│ • 155 Unit Tests         │   │ • 89 Unit Tests          │   │ • 88 Unit Tests          │
│ • 99.31% Line Coverage   │   │ • 94.5% Line Coverage    │   │ • 100% Line Coverage     │
│ • All 16 Controllers     │   │ • Model & Pipeline       │   │ • Validation & Nav       │
└──────────────────────────┘   └──────────────────────────┘   └──────────────────────────┘
```
...
* **Framework:** `Jest` (v29+)
* **Testing Pattern:** Unit testing Express controller methods in complete isolation. Service layer dependencies (`*.service.js`) and database models (`*.model.js`) are stubbed using `jest.mock()`.
...
### 1.3 Detailed Backend Test Suite Directory & Mapping
...
### 1.5 Detailed Coverage Metrics (Backend Layer Output)
...
-----------------------------|---------|----------|---------|---------|-------------------
 src/controllers             |   98.87 |    87.03 |     100 |   99.31 |                   
  ...
  report.controller.js       |   93.24 |    67.85 |     100 |   95.77 | 26-27,109         
...
## 🌿 Git Branching Strategy & Execution Cheat Sheet

### Branch Structure
* **`test` Branch:** Dedicated workspace branch containing all unit test files, mock factories, framework configs (`jest.config.js`, `vite.config.ts`), and coverage reports.

### Local Execution Command Cheat Sheet

```bash
# ---------------------------------------------------------
# 1. Backend REST API Tests (Node.js/Express - Jest)
# ---------------------------------------------------------
cd backend
npm test                          # Run all 155 unit tests
