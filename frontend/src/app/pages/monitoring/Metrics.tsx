import { useState, useEffect } from 'react';
import { Search, Server } from 'lucide-react';
import { Card, CardContent } from '../../components/ui/card';
import { Input } from '../../components/ui/input';
import { Label } from '../../components/ui/label';
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from '../../components/ui/select';
import { ComposedChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
import { hostService } from '../../services/hostService';
import { metricService, type ServerMetric } from '../../services/metricService';
import type { Host } from '../../types/host';
import { useLiveMetrics } from '../../hooks/useLiveMetrics';

export function Metrics() {
  const [searchQuery, setSearchQuery] = useState('');
  const [hosts, setHosts] = useState<Host[]>([]);
  const [selectedHostId, setSelectedHostId] = useState<string>('');
  const [metrics, setMetrics] = useState<ServerMetric[]>([]);

  const latestMetric = useLiveMetrics(selectedHostId ? parseInt(selectedHostId) : undefined);

  useEffect(() => {
    const loadHosts = async () => {
      try {
        const fetchedHosts = await hostService.getAll();
        setHosts(fetchedHosts);
        if (fetchedHosts.length > 0) {
          setSelectedHostId(fetchedHosts[0].server_id.toString());
        }
      } catch (err) {
        console.error("Failed to load hosts", err);
      }
    };
    loadHosts();
  }, []);

  useEffect(() => {
    if (!selectedHostId) return;
    const loadMetrics = async () => {
      try {
        const fetchedMetrics = await metricService.getServerMetrics(parseInt(selectedHostId), 60);
        setMetrics(fetchedMetrics);
      } catch (err) {
        console.error("Failed to load metrics", err);
      }
    };
    loadMetrics();
  }, [selectedHostId]);

  useEffect(() => {
    if (latestMetric) {
      setMetrics((prev) => {
        const newArray = [...prev, latestMetric];
        if (newArray.length > 60) newArray.shift();
        return newArray;
      });
    }
  }, [latestMetric]);

  const filteredHosts = hosts.filter(host =>
    host.hostname.toLowerCase().includes(searchQuery.toLowerCase()) ||
    host.environment.toLowerCase().includes(searchQuery.toLowerCase()) ||
    host.os.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const currentHost = hosts.find(h => h.server_id.toString() === selectedHostId);

  // Map server metrics to chart data formats
  const mapData = (key: keyof ServerMetric) => {
    return metrics.map((m) => {
      const val = Number(m[key]) || 0;
      return {
        time: new Date(m.recorded_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        actual: val,
      };
    });
  };

  const cpuData = metrics.length ? mapData('cpu_usage') : [];
  const memoryData = metrics.length ? mapData('memory_usage') : [];
  const diskData = metrics.length ? mapData('disk_usage') : [];
  const threadData = metrics.length ? mapData('thread_count') : [];

  const MetricChart = ({ 
    title, 
    data, 
    color
  }: { 
    title: string;
    data: any[];
    color: string;
  }) => {
    return (
      <Card className="bg-nebula-navy-light border-nebula-navy-lighter">
        <CardContent className="p-6">
          <h3 className="text-lg font-semibold text-white mb-4">{title}</h3>

          <ResponsiveContainer width="100%" height={240}>
            <ComposedChart data={data}>
              <defs>
                <linearGradient id={`gradient-${title}`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={color} stopOpacity={0.25}/>
                  <stop offset="95%" stopColor={color} stopOpacity={0.05}/>
                </linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" stroke="#1A1F3A" />
              <XAxis 
                dataKey="time" 
                stroke="#64748B" 
                style={{ fontSize: '12px' }}
                label={{ value: 'Hour', position: 'insideBottom', offset: -5, style: { fill: '#64748B', fontSize: '12px' } }}
              />
              <YAxis 
                stroke="#64748B" 
                style={{ fontSize: '12px' }}
                label={{ value: 'Messages', angle: -90, position: 'insideLeft', style: { fill: '#64748B', fontSize: '12px' } }}
              />
              <Tooltip 
                contentStyle={{ 
                  backgroundColor: '#0F172A', 
                  border: '1px solid #1E293B',
                  borderRadius: '8px',
                  color: '#F1F5F9',
                  fontSize: '12px'
                }}
              />
              
              {/* Actual Metric Line with Area */}
              <Area 
                type="monotone" 
                dataKey="actual" 
                stroke={color}
                strokeWidth={2}
                fill={`url(#gradient-${title})`}
                name="Current"
              />
            </ComposedChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
    );
  };

  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-semibold text-white">Host Metrics</h1>
        <p className="text-slate-400 text-sm mt-1">Monitor host performance metrics with baseline comparisons</p>
      </div>

      {/* Host Selection */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Search */}
        <Card className="bg-nebula-navy-light border-nebula-navy-lighter">
          <CardContent className="p-4">
            <Label className="text-white mb-2 block">Search Host</Label>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-slate-400" />
              <Input
                placeholder="Search by name, type, or location..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="pl-10 bg-nebula-navy-dark border-nebula-navy-lighter text-white placeholder:text-slate-500"
              />
            </div>
            {searchQuery && filteredHosts.length > 0 && (
              <div className="mt-2 max-h-48 overflow-y-auto space-y-1">
                {filteredHosts.map((host) => (
                  <div
                    key={host.server_id}
                    onClick={() => {
                      setSelectedHostId(host.server_id.toString());
                      setSearchQuery('');
                    }}
                    className="p-3 bg-nebula-navy-dark rounded cursor-pointer hover:bg-nebula-navy-lighter transition-colors"
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-white font-medium">{host.hostname}</p>
                        <p className="text-xs text-slate-400">{host.os} • {host.environment}</p>
                      </div>
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        host.server_status === 'HEALTHY' ? 'bg-green-500/10 text-green-400' :
                        host.server_status === 'WARNING' ? 'bg-yellow-500/10 text-yellow-400' :
                        'bg-red-500/10 text-red-400'
                      }`}>
                        {host.server_status}
                      </span>
                    </div>
                  </div>
                ))}
              </div>
            )}
          </CardContent>
        </Card>

        {/* Dropdown Selection */}
        <Card className="bg-nebula-navy-light border-nebula-navy-lighter">
          <CardContent className="p-4">
            <Label className="text-white mb-2 block">Select Host</Label>
            <Select value={selectedHostId} onValueChange={setSelectedHostId}>
              <SelectTrigger className="bg-nebula-navy-dark border-nebula-navy-lighter text-white">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="bg-nebula-navy-light border-nebula-navy-lighter text-white max-h-80">
                {hosts.map((host) => (
                  <SelectItem key={host.server_id} value={host.server_id.toString()}>
                    <div className="flex items-center gap-3">
                      <Server className="size-4 text-nebula-cyan" />
                      <div>
                        <p className="font-medium">{host.hostname}</p>
                        <p className="text-xs text-slate-400">{host.os} • {host.environment}</p>
                      </div>
                    </div>
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          </CardContent>
        </Card>
      </div>

      {/* Selected Host Info */}
      {currentHost && (
        <Card className="bg-gradient-to-r from-nebula-purple/10 to-nebula-blue/10 border-nebula-purple/30">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-nebula-purple to-nebula-blue flex items-center justify-center">
                  <Server className="size-7 text-white" />
                </div>
                <div>
                  <h2 className="text-xl font-semibold text-white">{currentHost.hostname}</h2>
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-sm text-slate-400">{currentHost.os}</span>
                    <span className="text-slate-600">•</span>
                    <span className="text-sm text-slate-400">{currentHost.environment}</span>
                  </div>
                </div>
              </div>
              <div>
                <span className={`px-4 py-2 rounded-lg text-sm font-medium ${
                  currentHost.server_status === 'HEALTHY' ? 'bg-green-500/20 text-green-400 border border-green-500/30' :
                  currentHost.server_status === 'WARNING' ? 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/30' :
                  'bg-red-500/20 text-red-400 border border-red-500/30'
                }`}>
                  {currentHost.server_status}
                </span>
              </div>
            </div>
          </CardContent>
        </Card>
      )}

      {/* Metric Charts */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <MetricChart
          title="CPU Utilization"
          data={cpuData}
          color="#3B82F6"
        />

        <MetricChart
          title="Memory Utilization"
          data={memoryData}
          color="#3B82F6"
        />

        <MetricChart
          title="Disk Usage"
          data={diskData}
          color="#3B82F6"
        />

        <MetricChart
          title="Thread Count"
          data={threadData}
          color="#3B82F6"
        />
      </div>
    </div>
  );
}
